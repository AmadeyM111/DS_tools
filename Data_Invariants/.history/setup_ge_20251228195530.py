...
try:
    ...
    # Проверяем, что мы в правильной директории (содержит setup_ge.py)
    if (script_dir / 'setup_ge.py').exists() or script_dir.name == 'Data_Invariants':
        PROJECT_ROOT = script_dir
    else:
        # Если скрипт запущен из другой директории, используем текущую рабочую директорию
        PROJECT_ROOT = Path.cwd().resolve()
except NameError:
    # Если __file__ недоступен (например, в интерактивном режиме), используем текущую директорию
    PROJECT_ROOT = Path.cwd().resolve()

# Логируем для отладки (будет видно в логах при запуске)
import logging
_temp_logger = logging.getLogger(__name__)
_temp_logger.debug(f"PROJECT_ROOT определен как: {PROJECT_ROOT}")
...
def get_config_from_env() -> dict:
    ...
    logger.debug(f"PROJECT_ROOT.parent: {PROJECT_ROOT.parent}")
    
    default_raw_data = PROJECT_ROOT.parent / 'data' / 'raw'
    default_clean_data = PROJECT_ROOT.parent / 'data' / 'processed'
    
    logger.debug(f"default_raw_data: {default_raw_data}")
    logger.debug(f"default_clean_data: {default_clean_data}")
    ...
    if raw_data_path:
        ...
        logger.debug(f"Путь из переменной окружения: {raw_data_path}")
    else:
        # Используем относительный путь от корня проекта
        raw_data_path = str(default_raw_data.resolve())
        logger.debug(f"Используется путь по умолчанию: {raw_data_path}")
    ...
    if clean_data_path:
        ...
        logger.debug(f"Путь к processed данным из переменной окружения: {clean_data_path}")
    else:
        clean_data_path = str(default_clean_data.resolve())
        logger.debug(f"Используется путь к processed данным по умолчанию: {clean_data_path}")
