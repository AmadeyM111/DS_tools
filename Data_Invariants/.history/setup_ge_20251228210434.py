...
def validate_data(context: Any, data_asset: Any, suite_name: str) -> Optional[Any]:
    ...
    try:
        ...
        if not checkpoint_exists:
            try:
                # Используем SimpleCheckpoint для создания checkpoint
                # Это правильный способ для fluent API в новых версиях Great Expectations
                ...
                logger.info(f"Checkpoint '{checkpoint_name}' успешно создан")
            except ImportError:
                # Если SimpleCheckpoint недоступен, пробуем альтернативный способ
                try:
                    # Альтернативный способ через checkpoint.run() напрямую
                    logger.info("SimpleCheckpoint недоступен, используем альтернативный способ валидации")
                    # Создаем валидатор и запускаем валидацию напрямую
                    validator = context.get_validator(
                        batch_request=batch_request,
                        expectation_suite_name=suite_name
                    )
                    ...
                    logger.error(f"Ошибка при альтернативной валидации: {e}")
                    raise
            except Exception as e:
                ...
                if "already exists" in str(e).lower() or "exists" in str(e).lower():
                    ...
                    logger.warning(f"Не удалось создать checkpoint через SimpleCheckpoint: {e}")
