...
def get_config_from_env() -> dict:
    ...
    if raw_data_path:
        ...
        # Важно: используем абсолютный путь напрямую, без resolve(), чтобы избежать проблем с символическими ссылками
    ...
    if clean_data_path:
        ...
        # Проверяем, что путь абсолютный
        if not Path(clean_data_path).is_absolute():
            clean_data_path = str(Path(clean_data_path).resolve())
