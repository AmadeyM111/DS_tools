        ...
        if not checkpoint_exists:
            try:
                ...
                try:
                    ...
                    # Явно получаем suite из контекста, чтобы убедиться, что он загружен
                    try:
                        suite = context.suites.get(suite_name)
                        logger.info(f"Suite '{suite_name}' загружен из контекста, содержит {len(suite.expectations)} expectations")
                    except Exception as suite_error:
                        logger.warning(f"Не удалось получить suite из контекста: {suite_error}, продолжаем с suite_name")
